...
{
    public partial class Retailer_Account : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (Button1.Text == "ADD")
            {
                if (b.Check_Retailer_Account(id) == 0)
                {
                    if (b.Insert_Retailer_Amount(id, int.Parse(txtAmt.Text)) > 0)
                    {
            ...
            {
                int amount = b.GetRet_Amount(id);
                int tt = amount + int.Parse(txtAmt.Text);
                if (b.Update_Retailer_Account(tt, id) > 0)
                {
        ...
        public void GetResult()
        {
            string em = Session["User"].ToString();
            int id = b.Get_Retailer_Id_Em(em);
            DataTable dt = b.Get_Retailer_Amount(id);
            if (dt.Rows.Count > 0)
            {
